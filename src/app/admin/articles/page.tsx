...
const showToast = (message: string, type: 'success' | 'error') => {
  ...
  toast.className = `fixed top-4 right-4 p-4 rounded-lg text-white ${type === 'success' ? 'bg-green-500' : 'bg-red-500'} transition-opacity duration-500`;
...
const ArticlesPage = () => {
  ...
  ), [searchText, categories, categoryFilter, statusFilter]);

  // 渲染文章行
  const renderArticleRow = useCallback((article: Article) => (
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {loading ? renderLoading() : filteredArticles.length === 0 ? renderEmpty() : (
              filteredArticles.map(article => renderArticleRow(article))
            )}
          </tbody>
        </table>
      </div>
